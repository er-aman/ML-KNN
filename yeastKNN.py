...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep  29 08:11:48 2019
...
def load_resample_data(fname,training_set=[],test_set=[]):
    with open(fname,'rb') as csvfile:
        ...
        for i in range (len(dataFromFile)):
            dataFromFile[i][0] = dataFromFile[i][0].split(' ')
            dataFromFile[i][0] = ' '.join(dataFromFile[i][0]).split()
            normalized_data.append(dataFromFile[i][0])
        # Calling cross validation for k as 10 folds 
        cross_validation(normalized_data,10)
...
def main():
   train_set=[]
   test_set=[]
   load_resample_data('yeast.data',train_set,test_set)
   averageAccuracy()
